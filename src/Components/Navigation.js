  ...
  let FoodItemArry = State.FoodList.length > 0 ? State.FoodList.map((i, k) => <>
            {i.Qty>0&&<>
              ...
              <div style={{ width: '50%', display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }} >
                <Button onClick={() => AddItemHandler(i.id)} variant="contained" color="primary">
                  <AddIcon />
                </Button>

                { i.totalItem===true && i.Qty>0 ? <>
                  <Button  onClick={() => RemoveItemHandler(i.id)} style={{ marginLeft: '10px' }} variant="contained" decreaseItems color="secondary">
                  <RemoveIcon />
                </Button>
                </>:<>
  ...
  return (
    <>
      ...
      >
        ...
        <DialogActions>
          <Button variant="contained" color="primary" onClick={GoToCheckout}>Save And CheckOut</Button>
